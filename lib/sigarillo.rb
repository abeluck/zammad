...
=end
  ...
  def self.timestamp_to_date(timestamp_str)
    return Time.at(timestamp_str.to_i).utc.to_datetime
  end


  def self.message_id(messageRaw)
    return "%s@%s" % [messageRaw['source'], messageRaw['timestamp']]
  end
...
=end
  ...
  def fetch_messages(group_id, channel)
    ...
    older_import = 0
    older_import_max = 20
    @api.fetch().each do |messageRaw|
      Rails.logger.debug {"processing message "}
      Rails.logger.debug {messageRaw.inspect}
      message = {
          source: messageRaw['source'],
          timestamp: messageRaw['timestamp'],
          created_at: Sigarillo.timestamp_to_date(messageRaw['timestamp']),
          id: Sigarillo.message_id(messageRaw),
          message: {
              body: messageRaw['message']['body'],
              profileKey: messageRaw['message']['profileKey'],
          }
      }
...
=end
  ...
  def to_user(message)
    ...
    if auth
      ...
      user = User.where(mobile: message[:source]).order(:updated_at).first
    end
    if user
